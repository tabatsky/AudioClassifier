import torch

from debug import _print

version_name = 'artist_v6'

artist_count = 3

lr = 3e-3
ktan = 0.3

sample_len = 24000

# S = 24000
# conv_kernel = 999
# conv_stride = 111
# conv_padding = 99
# => Sc = 209 + 1 = 210


def make_actual_nn():
    if version_name == 'artist':
        return ArtistNet()
    elif version_name == 'artist_v2':
        return ArtistNetV2()
    elif version_name == 'artist_v3':
        return ArtistNetV3()
    elif version_name == 'artist_v4':
        return ArtistNetV4()
    elif version_name == 'artist_v5':
        return ArtistNetV5()
    elif version_name == 'artist_v6':
        return ArtistNetV6()
    else:
        return None


class ArtistNet(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=36, conv1_padding=12,
                 conv1_kernel=99, conv1_stride=11,
                 pool1_kernel=17,
                 conv2_channels=36, conv2_kernel=8,
                 conv2_padding=4, conv2_stride=4,
                 pool2_kernel=33):
        super(ArtistNet, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride, padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride, padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.Tanh()
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.Tanh()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetV2(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=36, conv1_padding=12,
                 conv1_kernel=99, conv1_stride=11,
                 pool1_kernel=17,
                 conv2_channels=36, conv2_kernel=8,
                 conv2_padding=4, conv2_stride=4,
                 pool2_kernel=33):
        super(ArtistNetV2, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride, padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride, padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.Tanh()
        # self.activ1 = torch.nn.ReLU()
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.Tanh()
        # self.activ3 = torch.nn.ReLU()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N * (N + 2))
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=N+2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetV3(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=108, conv1_padding=12,
                 conv1_kernel=99, conv1_stride=11,
                 pool1_kernel=17,
                 conv2_channels=108, conv2_kernel=8,
                 conv2_padding=4, conv2_stride=4,
                 pool2_kernel=33,
                 dropout_p=0.04):
        super(ArtistNetV3, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        # self.bn = torch.nn.BatchNorm1d(1)
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride, padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride, padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        # self.activ1 = torch.nn.Tanh()
        self.activ1 = torch.nn.ELU()
        self.dropout1 = torch.nn.Dropout1d(dropout_p)
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.activ2 = torch.nn.ELU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ELU()
        self.dropout3 = torch.nn.Dropout1d(dropout_p)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N * (N + 2))
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=N+2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetV4(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=36, conv1_padding=12,
                 conv1_kernel=99, conv1_stride=11,
                 pool1_kernel=17,
                 conv2_channels=36, conv2_kernel=8,
                 conv2_padding=4, conv2_stride=4,
                 convA_kernel=17,
                 convA_padding=8, convA_stride=1,
                 pool2_kernel=33,
                 dropout_p=0.04):
        super(ArtistNetV4, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        # self.bn = torch.nn.BatchNorm1d(1)
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride,
                                     padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.convA1 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride,
                                      padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        # self.activ1 = torch.nn.Tanh()
        self.activ1 = torch.nn.ELU()
        # self.dropout1 = torch.nn.Dropout1d(dropout_p)
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.activ2 = torch.nn.ELU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ELU()
        # self.dropout3 = torch.nn.Dropout1d(dropout_p)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N * (N + 2))
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=N+2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        # x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        # x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetV5(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=18, conv1_padding=12,
                 conv1_kernel=99, conv1_stride=11,
                 pool1_kernel=17,
                 conv2_channels=18, conv2_kernel=8,
                 conv2_padding=4, conv2_stride=4,
                 convA_kernel=17,
                 convA_padding=8, convA_stride=1,
                 pool2_kernel=33,
                 dropout_p=0.04):
        super(ArtistNetV5, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        # self.bn = torch.nn.BatchNorm1d(1)
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride,
                                     padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.convA1 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA4 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA5 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride,
                                      padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        # self.activ1 = torch.nn.Tanh()
        self.activ1 = torch.nn.ELU()
        # self.dropout1 = torch.nn.Dropout1d(dropout_p)
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.activ2 = torch.nn.ELU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ELU()
        # self.dropout3 = torch.nn.Dropout1d(dropout_p)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N * (N + 2))
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=N+2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        # x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        # x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x
    
    
class ArtistNetV6(torch.nn.Module):
    def __init__(self, S=sample_len, N=artist_count,
                 conv1_channels=72, conv1_padding=7,
                 conv1_kernel=99, conv1_stride=5,
                 pool1_kernel=16,
                 conv2_channels=72, conv2_kernel=9,
                 conv2_padding=5, conv2_stride=12,
                 convA_kernel=9,
                 convA_padding=4, convA_stride=1,
                 pool2_kernel=26,
                 dropout_p=0.04):
        super(ArtistNetV6, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.S = S
        self.N = N

        self.Sc1 = (S - conv1_kernel + 2 * conv1_padding) // conv1_stride + 1
        print('Sc1', self.Sc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Sf1 = self.Sc1 // pool1_kernel
        print('Sf1', self.Sf1)
        self.Sc2 = (self.Sf1 - conv2_kernel + 2 * conv2_padding) // conv2_stride + 1
        print('Sc2', self.Sc2)
        self.Sf2 = self.Sc2 // pool2_kernel
        print('Sf2', self.Sf2)
        self.S2 = conv2_channels * self.Sf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        # self.bn = torch.nn.BatchNorm1d(1)
        self.conv1 = torch.nn.Conv1d(1, conv1_channels, conv1_kernel, stride=conv1_stride,
                                     padding=conv1_padding, groups=1)
        self.maxpool1 = torch.nn.MaxPool1d(kernel_size=pool1_kernel)
        self.convA1 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA4 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.convA5 = torch.nn.Conv1d(conv1_channels, conv1_channels, convA_kernel, stride=convA_stride,
                                      padding=convA_padding, groups=conv1_channels)
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, conv2_kernel, stride=conv2_stride,
                                      padding=conv2_padding, groups=conv1_channels)
        self.maxpool2 = torch.nn.MaxPool1d(kernel_size=pool2_kernel)
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        # self.activ1 = torch.nn.Tanh()
        self.activ1 = torch.nn.ELU()
        # self.dropout1 = torch.nn.Dropout1d(dropout_p)
        # self.conv2 = torch.nn.Conv2d(1, conv_channels, conv_kernel, stride=conv_stride, padding=conv_padding, groups=1)
        # self.maxpool2 = torch.nn.MaxPool2d(kernel_size=pool_kernel)
        # self.fc2 = torch.nn.Linear(self.Sl, self.n_hidden_neurons)
        # self.activ2 = torch.nn.Tanh()
        self.activ2 = torch.nn.ELU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ELU()
        # self.dropout3 = torch.nn.Dropout1d(dropout_p)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N * (N + 2))
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=N+2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.S)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        # x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        # x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        x = x.reshape(batch_size, self.N)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogram(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogram, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.Tanh()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.Tanh()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV2(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV2, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.Tanh()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.Tanh()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV3(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV3, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.conv_bn1 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activ1 = torch.nn.LeakyReLU()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA1 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA1 = torch.nn.LeakyReLU()
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA2 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA2 = torch.nn.LeakyReLU()
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA3 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA3 = torch.nn.LeakyReLU()
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA4 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA4 = torch.nn.LeakyReLU()
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA5 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA5 = torch.nn.LeakyReLU()
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.conv_bn1(x)
        x = self.conv_activ1(x)
        _print(x.shape)
        bns = [self.conv_bnA1, self.conv_bnA2, self.conv_bnA3, self.conv_bnA4, self.conv_bnA5]
        activs = [self.conv_activA1, self.conv_activA2, self.conv_activA3, self.conv_activA4, self.conv_activA5]
        for i, conv in enumerate([self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]):
            x1 = conv(x)
            x = x + x1
            x = bns[i](x)
            x = activs[i](x)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV4(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV4, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.conv_bn1 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activ1 = torch.nn.LeakyReLU()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA1 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA1 = torch.nn.LeakyReLU()
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA2 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA2 = torch.nn.LeakyReLU()
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA3 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA3 = torch.nn.LeakyReLU()
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA4 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA4 = torch.nn.LeakyReLU()
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.conv_bnA5 = torch.nn.BatchNorm2d(num_features=conv1_channels)
        self.conv_activA5 = torch.nn.LeakyReLU()
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dropout1 = torch.nn.Dropout(0.3)
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dropout3 = torch.nn.Dropout(0.3)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.conv_bn1(x)
        x = self.conv_activ1(x)
        _print(x.shape)
        bns = [self.conv_bnA1, self.conv_bnA2, self.conv_bnA3, self.conv_bnA4, self.conv_bnA5]
        activs = [self.conv_activA1, self.conv_activA2, self.conv_activA3, self.conv_activA4, self.conv_activA5]
        for i, conv in enumerate([self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]):
            x1 = conv(x)
            x = x + x1
            x = bns[i](x)
            x = activs[i](x)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x)
        x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x)
        x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV5(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV5, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.Tanh()
        self.dropout1 = torch.nn.Dropout(0.5)
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.Tanh()
        self.dropout3 = torch.nn.Dropout(0.5)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV6(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV6, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.activ_c1 = torch.nn.LeakyReLU()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA1 = torch.nn.LeakyReLU()
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA2 = torch.nn.LeakyReLU()
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA3 = torch.nn.LeakyReLU()
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA4 = torch.nn.LeakyReLU()
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA5 = torch.nn.LeakyReLU()
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.activ_c2 = torch.nn.LeakyReLU()
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dropout1 = torch.nn.Dropout(0.5)
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dropout3 = torch.nn.Dropout(0.5)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.activ_c1(x)
        _print(x.shape)
        activs = [self.activ_cA1, self.activ_cA2, self.activ_cA3, self.activ_cA4, self.activ_cA5]
        for i, conv in enumerate([self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]):
            x1 = activs[i](conv(x))
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.activ_c2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV7(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=18,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=3,
                 convA_padding=1, convA_stride=1,
                 conv2_channels=18,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV7, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv0 = torch.nn.Conv2d(1, conv1_channels,
                                     (1, 1), stride=(1, 1),
                                     padding=(0, 0), groups=1)
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA1 = torch.nn.LeakyReLU()
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA2 = torch.nn.LeakyReLU()
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA3 = torch.nn.LeakyReLU()
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA4 = torch.nn.LeakyReLU()
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.activ_cA5 = torch.nn.LeakyReLU()
        self.conv1 = torch.nn.Conv2d(conv1_channels, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.activ_c1 = torch.nn.LeakyReLU()
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.activ_c2 = torch.nn.LeakyReLU()
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dropout1 = torch.nn.Dropout(0.5)
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dropout3 = torch.nn.Dropout(0.5)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv0(x)
        _print(x.shape)
        activs = [self.activ_cA1, self.activ_cA2, self.activ_cA3, self.activ_cA4, self.activ_cA5]
        for i, conv in enumerate([self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]):
            x1 = activs[i](conv(x))
            x = x + x1
        x = self.conv1(x)
        x = self.activ_c1(x)
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.activ_c2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dropout1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dropout3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV8(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV8, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.ReLU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ReLU()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.fc4(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


import torch
import torch.nn as nn
import torch.nn.functional as F


# class XPUDropout(nn.Module):
#     def __init__(self, p=0.5, inplace=False):
#         super().__init__()
#         self.p = p
#         self.inplace = inplace
#
#     def forward(self, input):
#         if self.training and self.p > 0:
#             # Custom implementation for XPU
#             if input.device.type == 'xpu':
#                 # Create mask on XPU
#                 mask = torch.rand_like(input) > self.p
#                 zero_tensor = torch.FloatTensor([0.0]).to(input.device)
#                 one_tensor = torch.FloatTensor([1.0]).to(input.device)
#                 mask = torch.where(mask, one_tensor, zero_tensor)
#
#                 # Scale output
#                 # output = input * mask / (1 - self.p)
#                 output = input * mask
#                 return output
#             else:
#                 # Use PyTorch's native implementation for other devices
#                 return F.dropout2d(input, self.p, self.training, self.inplace)
#         return input


class ArtistNetSpectrogramV9(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV9, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.ReLU()
        self.dp1 = torch.nn.Dropout(0.5)
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.ReLU()
        self.dp2 = torch.nn.Dropout(0.5)
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = conv(x)
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV10(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV10, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        print('n_hidden_neurons', self.n_hidden_neurons)

        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.bn1 = torch.nn.BatchNorm2d(conv1_channels)

        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA1 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA2 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA3 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA4 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA5 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA6 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA6 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA7 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA7 = torch.nn.BatchNorm2d(conv1_channels)

        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))

        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.bn2 = torch.nn.BatchNorm2d(conv2_channels)

        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))

        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.bnfc1 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ1 = torch.nn.ReLU()
        self.dp1 = torch.nn.Dropout(0.5)

        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc3 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ3 = torch.nn.ReLU()
        self.dp2 = torch.nn.Dropout(0.5)

        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.bn1(x)
        _print(x.shape)

        for conv, bn in [(self.convA1, self.bnA1), (self.convA2, self.bnA2),
                         (self.convA3, self.bnA3), (self.convA4, self.bnA4),
                         (self.convA5, self.bnA5), (self.convA6, self.bnA6),
                         (self.convA7, self.bnA7)]:
            x1 = bn(conv(x))
            x = x + x1

        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.bn2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        _print(x.shape)
        x = self.fc1(x)
        x = self.bnfc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        x = self.bnfc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV11(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV11, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        print('n_hidden_neurons', self.n_hidden_neurons)

        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.bn1 = torch.nn.BatchNorm2d(conv1_channels)

        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA1 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA2 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA3 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA4 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA5 = torch.nn.BatchNorm2d(conv1_channels)

        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))

        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.bn2 = torch.nn.BatchNorm2d(conv2_channels)

        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))

        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.bnfc1 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dp1 = torch.nn.Dropout(0.5)

        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc3 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dp2 = torch.nn.Dropout(0.5)

        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.bn1(x)
        _print(x.shape)

        for conv, bn in [(self.convA1, self.bnA1), (self.convA2, self.bnA2),
                         (self.convA3, self.bnA3), (self.convA4, self.bnA4),
                         (self.convA5, self.bnA5)]:
            x1 = bn(conv(x))
            x = x + x1

        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.bn2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        _print(x.shape)
        x = self.fc1(x)
        x = self.bnfc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        x = self.bnfc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV12(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV12, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 6
        print('n_hidden_neurons', self.n_hidden_neurons)

        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.bn1 = torch.nn.BatchNorm2d(conv1_channels)

        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA1 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA2 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA3 = torch.nn.BatchNorm2d(conv1_channels)

        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))

        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.bn2 = torch.nn.BatchNorm2d(conv2_channels)

        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))

        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.bnfc1 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dp1 = torch.nn.Dropout(0.5)

        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc3 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dp2 = torch.nn.Dropout(0.5)

        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc4 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ4 = torch.nn.LeakyReLU()
        self.dp3 = torch.nn.Dropout(0.5)

        self.fc5 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.bn1(x)
        _print(x.shape)

        for conv, bn in [(self.convA1, self.bnA1), (self.convA2, self.bnA2),
                         (self.convA3, self.bnA3)]:
            x1 = bn(conv(x))
            x = x + x1

        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.bn2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        _print(x.shape)
        x = self.fc1(x)
        x = self.bnfc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        x = self.bnfc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        x = self.bnfc4(x)
        _print(x.shape)
        x = self.activ4(x * ktan) / ktan
        x = self.dp3(x)
        x = self.fc5(x)
        _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV13(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV13, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 2
        print('n_hidden_neurons', self.n_hidden_neurons)

        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.bn1 = torch.nn.BatchNorm2d(conv1_channels)

        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA1 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA2 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA3 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA4 = torch.nn.BatchNorm2d(conv1_channels)

        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))

        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.bn2 = torch.nn.BatchNorm2d(conv2_channels)

        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))

        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.bnfc1 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dp1 = torch.nn.Dropout(0.5)

        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc3 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dp2 = torch.nn.Dropout(0.5)

        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc4 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ4 = torch.nn.LeakyReLU()
        self.dp3 = torch.nn.Dropout(0.5)

        self.fc5 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.bn1(x)
        _print(x.shape)

        for conv, bn in [(self.convA1, self.bnA1), (self.convA2, self.bnA2),
                         (self.convA3, self.bnA3), (self.convA4, self.bnA4)]:
            x1 = bn(conv(x))
            x = x + x1

        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.bn2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        _print(x.shape)
        x = self.fc1(x)
        x = self.bnfc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        x = self.bnfc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        x = self.bnfc4(x)
        _print(x.shape)
        x = self.activ4(x * ktan) / ktan
        x = self.dp3(x)
        x = self.fc5(x)
        _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV14(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=2, conv1_Hpadding=3,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=2,
                 pool1_Wkernel=6, pool1_Hkernel=7,
                 convA_Wkernel=9, convA_Hkernel=7,
                 convA_Wpadding=4, convA_Hpadding=3,
                 convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=1,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_Wkernel=11, pool2_Hkernel=8):
        super(ArtistNetSpectrogramV14, self).__init__()

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        self.Wf1 = self.Wc1 // pool1_Wkernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_Hkernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_Wkernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_Hkernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2
        print('n_hidden_neurons', self.n_hidden_neurons)

        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.bn1 = torch.nn.BatchNorm2d(conv1_channels)

        self.activA = torch.nn.Tanh()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA1 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA2 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)
        self.bnA3 = torch.nn.BatchNorm2d(conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_Wkernel, convA_Hkernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=(convA_Wpadding, convA_Hpadding), groups=conv1_channels)

        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_Wkernel, pool1_Hkernel))
        self.bnA4 = torch.nn.BatchNorm2d(conv1_channels)

        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.bn2 = torch.nn.BatchNorm2d(conv2_channels)

        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_Wkernel, pool2_Hkernel))

        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.bnfc1 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.dp1 = torch.nn.Dropout(0.5)

        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc3 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.dp2 = torch.nn.Dropout(0.5)

        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.bnfc4 = torch.nn.BatchNorm1d(self.n_hidden_neurons)
        self.activ4 = torch.nn.LeakyReLU()
        self.dp3 = torch.nn.Dropout(0.5)

        self.fc5 = torch.nn.Linear(self.n_hidden_neurons, N)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        x = self.bn1(x)
        _print(x.shape)

        for conv, bn in [(self.convA1, self.bnA1), (self.convA2, self.bnA2),
                         (self.convA3, self.bnA3), (self.convA4, self.bnA4)]:
            x1 = conv(self.activA(bn(x) * ktan) / ktan)
            x = x + x1

        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        x = self.bn2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        _print(x.shape)
        x = self.fc1(x)
        x = self.bnfc1(x)
        _print(x.shape)
        x = self.activ1(x * ktan) / ktan
        x = self.dp1(x)
        x = self.fc3(x)
        x = self.bnfc3(x)
        _print(x.shape)
        x = self.activ3(x * ktan) / ktan
        x = self.dp2(x)
        x = self.fc4(x)
        x = self.bnfc4(x)
        _print(x.shape)
        x = self.activ4(x * ktan) / ktan
        x = self.dp3(x)
        x = self.fc5(x)
        _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV15(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=36,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=36,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV15, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 2
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.activA = torch.nn.LeakyReLU()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA4 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA5 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3, self.convA4, self.convA5]:
            x1 = self.activA(conv(x))
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x


class ArtistNetSpectrogramV16(torch.nn.Module):
    def __init__(self, W=129, H=107, N=artist_count,
                 conv1_channels=108,
                 conv1_Wpadding=3, conv1_Hpadding=0,
                 conv1_Wkernel=3, conv1_Hkernel=3,
                 conv1_stride=3,
                 pool1_kernel=5,
                 convA_kernel=5,
                 convA_padding=2, convA_stride=1,
                 conv2_channels=108,
                 conv2_Wpadding=1, conv2_Hpadding=2,
                 conv2_Wkernel=3, conv2_Hkernel=3,
                 conv2_stride=1,
                 pool2_kernel=9):
        super(ArtistNetSpectrogramV16, self).__init__()

        # self.conv1_channels = conv1_channels
        self.pool1_kernel = pool1_kernel
        # self.conv2_channels = conv2_channels
        self.pool2_kernel = pool2_kernel

        self.W = W
        self.H = H
        self.N = N

        self.Wc1 = (W - conv1_Wkernel + 2 * conv1_Wpadding) // conv1_stride + 1
        print('Wc1', self.Wc1)
        self.Hc1 = (H - conv1_Hkernel + 2 * conv1_Hpadding) // conv1_stride + 1
        print('Hc1', self.Hc1)
        # self.S1 = conv1_channels * self.Sc1 // pool1_kernel
        # print('S1', self.S1)
        self.Wf1 = self.Wc1 // pool1_kernel
        print('Wf1', self.Wf1)
        self.Hf1 = self.Hc1 // pool1_kernel
        print('Hf1', self.Hf1)
        self.Wc2 = (self.Wf1 - conv2_Wkernel + 2 * conv2_Wpadding) // conv2_stride + 1
        print('Wc2', self.Wc2)
        self.Hc2 = (self.Hf1 - conv2_Hkernel + 2 * conv2_Hpadding) // conv2_stride + 1
        print('Hc2', self.Hc2)
        self.Wf2 = self.Wc2 // pool2_kernel
        print('Sf2', self.Wf2)
        self.Hf2 = self.Hc2 // pool2_kernel
        print('Hf2', self.Hf2)
        self.S2 = conv2_channels * self.Wf2 * self.Hf2
        print('S2', self.S2)
        self.n_hidden_neurons = self.S2 * 2
        # self.n_hidden_neurons = 240
        # self.n_hidden_neurons = self.S1
        print('n_hidden_neurons', self.n_hidden_neurons)

        # self.fc0 = torch.nn.Linear(self.S, self.n_hidden_neurons)
        # self.activ0 = torch.nn.Tanh()
        self.conv1 = torch.nn.Conv2d(1, conv1_channels,
                                     (conv1_Wkernel, conv1_Hkernel), stride=(conv1_stride, conv1_stride),
                                     padding=(conv1_Wpadding, conv1_Hpadding), groups=1)
        self.activA = torch.nn.LeakyReLU()
        self.convA1 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA2 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.convA3 = torch.nn.Conv2d(conv1_channels, conv1_channels, (convA_kernel, convA_kernel),
                                      stride=(convA_stride, convA_stride),
                                      padding=convA_padding, groups=conv1_channels)
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=(pool1_kernel, pool1_kernel))
        self.conv2 = torch.nn.Conv2d(conv1_channels, conv2_channels,
                                     (conv2_Wkernel, conv2_Hkernel), stride=(conv2_stride, conv2_stride),
                                     padding=(conv2_Wpadding, conv2_Hpadding), groups=1)
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=(pool2_kernel, pool2_kernel))
        self.fc1 = torch.nn.Linear(self.S2, self.n_hidden_neurons)
        self.activ1 = torch.nn.LeakyReLU()
        self.fc3 = torch.nn.Linear(self.n_hidden_neurons, self.n_hidden_neurons)
        self.activ3 = torch.nn.LeakyReLU()
        self.fc4 = torch.nn.Linear(self.n_hidden_neurons, 2*N)
        self.maxpool3 = torch.nn.MaxPool1d(kernel_size=2)
        self.sm = torch.nn.Softmax(dim=1)

    def forward(self, x):
        _print('forward')
        batch_size = list(x.shape)[0]
        x = x.reshape(batch_size, 1, self.W, self.H)
        x = self.conv1(x)
        _print(x.shape)
        for conv in [self.convA1, self.convA2, self.convA3]:
            x1 = self.activA(conv(x))
            x = x + x1
        x = self.maxpool1(x)
        _print(x.shape)
        x = self.conv2(x)
        _print(x.shape)
        x = self.maxpool2(x)
        _print(x.shape)
        x = x.reshape(batch_size, -1)
        # x = x.transpose(1, 2)
        _print(x.shape)
        x = self.fc1(x)
        _print(x.shape)
        x = self.activ1(x)
        x = self.fc3(x)
        _print(x.shape)
        x = self.activ3(x)
        x = self.fc4(x)
        _print(x.shape)
        x = self.maxpool3(x)
        _print(x.shape)
        # x = x.reshape(batch_size, -1)
        # _print(x.shape)
        # x = self.sm(x)
        # _print(x.shape)
        return x

    def inference(self, x):
        x = self.forward(x)
        x = self.sm(x)
        return x