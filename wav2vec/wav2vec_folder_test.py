...
from deepseek_wav2vec_test import AudioClassifier
...
def detect_file_type(fn_in_mp3):
    ...
    classifier = AudioClassifier("wav2vec2_finetuned")
    result = classifier.predict(fn_in_mp3, return_probs=True)

    if result:
        class_name, confidence, probs = result
        print(f"Результат: {class_name} (уверенность: {confidence:.2%})")
        ...
        for idx, prob in enumerate(probs):
            ...
            print(f"  {idx}: {prob:.2%}")
        idx = np.argmax(np.array(all_probs))
        idx = all_idx[idx]
        return [idx, all_probs[idx]]
